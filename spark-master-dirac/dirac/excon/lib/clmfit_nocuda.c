...
static void
gauss_fn(double *p,double *x, int M, int N, void *data) {
 lmgrid_data *t=(lmgrid_data*)data;
 double l,m;
 int i;
 double sinp,cosp;
 for (i=0; i<N;i++) {
  sincos(p[2],&sinp,&cosp);
  l=t->lgrid[i]*cosp+t->mgrid[i]*sinp;
  m=-t->lgrid[i]*sinp+t->mgrid[i]*cosp;
  l/=p[0];
  m/=p[1];
  x[i]=exp(-l*l-m*m);
 }
...
fit_gaussian_psf(double *psf,double *lgrid,double *mgrid, int N, double *bmaj, double *bmin, double *bpa) {
 ...
 ret=clevmar_der_single_nocuda(gauss_fn, NULL, p, psf, m, N, 400, NULL,NULL, 2, &t); 
